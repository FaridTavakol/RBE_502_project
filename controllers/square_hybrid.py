...
class KukaController:
	...
	def initialize_controller(self):
		...
		## Joint Space
		self.prev_state_PD = np.zeros(5) #the last 4 joints are controller using PID
		self.prev_vel_PD = np.zeros(5)
		self.prev_vel1_PD = np.zeros(5)
		self.prev_vel2_PD = np.zeros(5)
		self.prev_vel3_PD = np.zeros(5)
		self.prev_vel4_PD = np.zeros(5)
		self.prev_wt_vel_PD = np.zeros(5)
	...
	def generate_joint_trajectory(self, q):
		...
		q_des = q + self.joint_speed * (self.dt)#+0.03
...
# ============================ PD portion =================================
		...
		# stateGoal_PD = np.array([0, 0, 0.0, 0.0, 0.0])
